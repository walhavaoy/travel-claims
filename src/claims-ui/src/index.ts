...
const logger = pino({ name: "claims-ui" });
...
logger.info({ version: VERSION }, "claims-ui version");
...
const bundleEntry = staticCache.get("/ui/claims.js");
...
const server = createServer((req, res) => {
  ...
  if (url === "/ui/claims-manifest.json") {
...
server.listen(PORT, () => {
  logger.info({ port: PORT }, "claims-ui listening");
});
